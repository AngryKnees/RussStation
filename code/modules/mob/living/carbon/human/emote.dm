/mob/living/carbon/human/emote(var/act,var/m_type=1,var/message = null,var/force)
	...
	switch(act)
		...
		if("fart", "farts", "flip", "flips", "snap", "snaps")
			on_CD = handle_emote_CD(50)				//proc located in code\modules\mob\emote.dm
	...
	switch(act)
		...
		if("fart", "farts")
			...
			if(mind.assigned_role == "Clown")
				...
				message = "<b>[src]</b> [pick("toots", "poots")] and confetti flies out!"
				var/spawner_type = /obj/effect/decal/cleanable/confetti
				playsound(loc, 'sound/effects/confetti_partywhistle.ogg', 50, 1)
				for(var/i in 1 to 2) //1 to 2 confettis.
			...
			playsound(loc, "[dna.species.fart_sound]", 50, 1)
			m_type = 2
